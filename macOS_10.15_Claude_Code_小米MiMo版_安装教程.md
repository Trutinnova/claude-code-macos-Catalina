# 老 Mac（macOS 10.15）安装 Claude Code 教程（小米 MiMo 版）

## 开始之前先搞清楚

这篇教程写给谁看的：
- 你的 Mac 是 2015~2019 年的老款，系统停在 10.15（Catalina），升级不了
- 你用的是**小米 MiMo 大模型**，想把它接到 Claude Code 上写代码
- 你去网上搜安装教程，跟着敲命令结果报错 `dyld: Symbol not found`
- 你不想折腾技术细节，只想照着做、能用就行

这篇教程不废话，每一步都告诉你**贴什么、回车、看什么结果**。

> 如果你用的是 DeepSeek，请找 DeepSeek 版教程，别用这篇。
...
打开 `platform.xiaomimimo.com`，注册，登录。进去后找"API Keys"，创建一个新 Key，复制保存好。是一串以 `sk-` 开头的字符。**这串东西就是你的钥匙，不要告诉别人。**
...
## 让 Claude Code 能连上小米 MiMo

#### 第 1 步：设置钥匙

下面每条命令**一行一行贴**，把里面的 `sk-你的Key` 换成你在 MiMo 后台拿到的真实 Key：
...
> MiMo 比 DeepSeek 多了两条（`CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1` 和 `CLAUDE_CODE_SIMPLE=1`），这是防止 API 报 400 错误的关键，不能漏。
...
上面的设置关掉终端就没了。把下面 11 条命令**一条一条贴**，写入配置文件。跟前面一样，贴一行回车一下：
...
```
echo 'export ANTHROPIC_BASE_URL=https://api.xiaomimimo.com/anthropic' >> ~/.zshrc
```
...
```
echo 'export ANTHROPIC_MODEL=mimo-v2.5-pro' >> ~/.zshrc
```

```
echo 'export ANTHROPIC_DEFAULT_HAIKU_MODEL=mimo-v2-flash' >> ~/.zshrc
```

```
echo 'export ANTHROPIC_DEFAULT_SONNET_MODEL=mimo-v2.5-pro' >> ~/.zshrc
```

```
echo 'export ANTHROPIC_DEFAULT_OPUS_MODEL=mimo-v2.5-pro' >> ~/.zshrc
```

```
echo 'export CLAUDE_CODE_SUBAGENT_MODEL=mimo-v2-flash' >> ~/.zshrc
```
...
| `API Error: 400 messages[N].role: got 'system'` | MiMo 不认 system 角色 | 确认 `CLAUDE_CODE_SIMPLE=1` 和 `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1` 设了没 |
...
```
sed -i '' '/^export ANTHROPIC_/d; /^export DISABLE_AUTOUPDATER=/d; /^export API_TIMEOUT_MS=/d; /^export CLAUDE_CODE_SUBAGENT_MODEL=/d; /^export CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=/d; /^export CLAUDE_CODE_SIMPLE=/d' ~/.zshrc
```
...
- 为什么比 DeepSeek 多了两个参数：MiMo 对请求格式更严格，`CLAUDE_CODE_SIMPLE=1` 和 `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1` 是为了避免 `system` 角色导致的 400 错误
- API Key 在 https://platform.xiaomimimo.com 获取，按用量扣费，不写代码的时候不花钱
